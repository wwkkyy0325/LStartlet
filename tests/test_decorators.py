...
from core.decorators import (
    with_error_handling, with_logging, with_error_handling_async, with_logging_async,
    cached_async, require_permission, require_permission_async, monitor_metrics,
...
class TestPermissionDecorators(unittest.TestCase):
    ...
    def test_require_permission_success(self):
        """测试权限检查成功"""
        with patch('core.decorators._get_current_user_permission_level', return_value=PermissionLevel.ADMIN):
    ...
    def test_require_permission_failure(self):
        """测试权限检查失败"""
        with patch('core.decorators._get_current_user_permission_level', return_value=PermissionLevel.USER):
...
class TestMonitorMetricsDecorators(unittest.TestCase):
    ...
    def test_monitor_metrics_failure(self):
        ...
        collector = MetricsCollector()
        # 查找包含错误信息的键
        failure_keys = [k for k in collector._metrics.keys() if 'failure' in k and 'ValueError' in k]
...
class TestOtherDecorators(unittest.TestCase):
    ...
    def test_plugin_component(self):
        ...
        self.assertEqual(getattr(TestPluginComponent, '_plugin_component_id', None), "test_component")
    ...
    def test_plugin_event_handler(self):
        ...
        self.assertEqual(getattr(test_handler, '_handled_event_type', None), "test.event.type")
