...
class TestConfigManagerInternalMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建一个干净的配置管理器实例，不加载任何配置文件
        self.config_manager = ConfigManager()
        # 清空所有配置以确保测试隔离
        self.config_manager._config.clear()
        self.config_manager._config_sources.clear()
    ...
    def test_register_config_with_source(self):
        ...
        self.assertEqual(
            self.config_manager.get_config("test_internal_config"), "internal_value"
        )
    ...
    def test_get_config_source(self):
        ...
        # 系统配置项应该有 "system" 来源
        system_source = self.config_manager.get_config_source("system_test_config")
        self.assertEqual(system_source, "system")

        # 外部配置项应该有 "external" 来源
        ...
        external_source = self.config_manager.get_config_source(
            "unique_external_test_config_12345"
        )
        self.assertEqual(external_source, "external")

        # 不存在的配置项应该返回 None
        non_existent_source = self.config_manager.get_config_source(
            "non_existent_config"
        )
    ...
    def test_get_all_configs_by_source(self):
        ...
        self.assertIn("system_test_config", system_configs)
