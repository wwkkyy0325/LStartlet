...
class DependencyInstaller:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        # 注册依赖安装相关配置
        register_config(
            "dependency_installer.auto_install", True, bool, "是否自动安装缺失的依赖"
        )
        register_config(
            "dependency_installer.pip_index_url",
        ...
        register_config(
            "dependency_installer.requirements_file",
            "requirements.txt",
            str,
            "requirements文件路径",
        )
    ...
    def check_and_install_missing(
        self, required_packages: Optional[Dict[str, str]] = None
    ) -> bool:
        """
        检查并安装缺失的依赖
        ...
        if required_packages is None:
            # 从requirements文件读取依赖
            requirements_file = get_config(
                "dependency_installer.requirements_file", "requirements.txt"
            )
        ...
        info(
            f"发现 {len(missing_packages)} 个缺失的依赖: {list(missing_packages.keys())}"
        )
    ...
    def _is_package_installed(self, package_name: str, version_req: str = "") -> bool:
        ...
        """
        try:
            # 首先尝试使用 importlib.metadata (Python 3.8+) 来检查包信息
            # 这是最安全的方法，不会实际导入模块
            try:
                ...
                # 其他异常（如 PackageNotFoundError）也回退到其他方法
                # 注意：我们不能直接引用 metadata.PackageNotFoundError
                # 因为在 ImportError 的情况下 metadata 未定义
                pass

            # 如果 importlib.metadata 找不到，回退到模块导入方式
            # 但不再有特殊处理逻辑
    ...
    def _check_version_match(self, installed_version: str, version_req: str) -> bool:
        ...
        try:
            ...
            # 简单的版本比较
            inst_parts = [
                int(x) for x in re.split(r"[^\d]", installed_version)[:3] if x.isdigit()
            ]
    ...
    def install_packages(self, packages: Dict[str, str]) -> bool:
        ...
        index_url = get_config(
            "dependency_installer.pip_index_url", "https://pypi.org/simple/"
        )
        if index_url:
        ...
        try:
            result = subprocess.run(
                install_args, capture_output=True, text=True, check=True
            )
