...
class PersistenceError(Exception):
    ...
    def __init__(
        self, operation: str, message: str, inner_exception: Optional[Exception] = None
    ):
...
class DataSerializationError(PersistenceError):
    ...
    def __init__(
        self, data_type: str, message: str, inner_exception: Optional[Exception] = None
    ):
        super().__init__(
            "serialize",
            f"数据类型 '{data_type}' 序列化失败: {message}",
            inner_exception,
        )
