...
class SystemConfigManager:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        # 注册配置管理相关配置
        register_config(
            "system_config.auto_apply", True, bool, "是否自动应用系统检测配置"
        )
        register_config(
            "system_config.config_file", "./system_config.yaml", str, "系统配置文件路径"
        )
    ...
    def setup_optimal_config(self) -> bool:
        ...
        try:
            ...
            # 验证系统要求
            validation_result = self.system_detector.validate_system_requirements(
                system_info
            )
            ...
            # 根据系统信息生成配置
            optimal_config = self.system_detector.generate_config_from_system(
                system_info
            )
    ...
    def _apply_config(self, config: Dict[str, Any]) -> None:
        ...
        for key, value in config.items():
            ...
            # 使用配置管理器设置配置项
            from LStartlet.core.config import get_config_manager
    ...
    def _save_config_to_file(
        self, config: Dict[str, Any], system_info: Dict[str, Any]
    ) -> None:
        """将配置保存到文件"""
        ...
        if get_config("system_config.backup_config", True) and os.path.exists(
            config_file
        ):
    ...
    def load_config_from_file(
        ...
        if not config_file:
            config_file = get_config(
                "system_config.config_file", "./system_config.yaml"
            )
    ...
    def get_recommended_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 根据系统信息生成配置
            recommended_config = self.system_detector.generate_config_from_system(
                system_info
            )
