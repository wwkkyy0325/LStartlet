...
# 使用项目自定义日志管理器
from LStartlet.core.logger import info, warning, error, debug

# 使用事件系统
from LStartlet.core.event.events.scheduler_events import TickEvent
...
class TickComponent:
    ...
    def __init__(
        self, config: Optional[TickConfig] = None, event_bus: Optional[EventBus] = None
    ):
        """
        初始化Tick组件
        ...
        try:
            while self._state == TickState.RUNNING:
                ...
                if (
                    ...
                    and self._current_tick >= self.config.max_ticks
                ):
                    break
    ...
    async def _execute_tick(self) -> None:
        ...
        if self._event_bus is not None:
            tick_data: Dict[str, Any] = {
                ...
                "callback_count": len(self._tick_callbacks)
                + len(self._async_tick_callbacks),
            }
            self._event_bus.publish(
                TickEvent(self._current_tick, elapsed_time, tick_data)
            )
