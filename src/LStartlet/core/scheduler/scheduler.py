...
from LStartlet.core.error.exceptions import ProcessingError, ConfigError
...
# 依赖注入容器
from LStartlet.core.di.app_container import get_app_container  # type: ignore
from LStartlet.core.config.config_manager import (
...
class Scheduler:
    ...
    def __init__(self, config: Optional[SchedulerConfig] = None):
        ...
        if scheduler_config_dict is None:
            ...
            scheduling_strategy = scheduler_config_dict.get(
                "scheduling_strategy", "round_robin"
            )

        # 初始化所有组件
        self._process_manager = ProcessManager(
            max_processes=max_processes, process_timeout=process_timeout
        )
        self._task_dispatcher = TaskDispatcher(
            max_concurrent_tasks=max_concurrent_tasks, strategy=scheduling_strategy
        )
    ...
    def config(self) -> SchedulerConfig:
        ...
        config.max_concurrent_tasks = scheduler_config_dict.get(
            "max_concurrent_tasks", 10
        )
        config.scheduling_strategy = scheduler_config_dict.get(
            "scheduling_strategy", "round_robin"
        )
        return config
    ...
    def start(self) -> None:
        ...
        try:
            ...
            try:
                ...
                warning(
                    "No running event loop detected, skipping tick component startup"
                )
        ...
        try:
            ...
            if config_dict is not None:
                ...
                if hasattr(self._process_manager, "max_processes"):
                    self._process_manager.max_processes = config_dict.get(
                        "max_processes", 4
                    )
                if hasattr(self._process_manager, "process_timeout"):
                    self._process_manager.process_timeout = config_dict.get(
                        "process_timeout", 30.0
                    )
                if hasattr(self._task_dispatcher, "max_concurrent_tasks"):
                    self._task_dispatcher.max_concurrent_tasks = config_dict.get(
                        "max_concurrent_tasks", 10
                    )
            ...
            self.event_bus.publish(
                SchedulerStatusEvent(
                    "config_updated",
                    {"updated_config": kwargs, "current_config": config_dict or {}},
                )
            )
        except Exception as e:
    ...
    async def submit_task(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            handle_error(
                ProcessingError(
                    error_msg, context={"task_id": task_id, "priority": priority}
                )
            )
            raise
    ...
    async def run_scheduler_loop(self) -> None:
        ...
        try:
            ...
            while self._is_running:
                # Dispatch tasks
                while (
                    self._task_dispatcher.get_queue_size() > 0
                    and self._task_dispatcher.get_active_task_count()
                    < self._task_dispatcher.max_concurrent_tasks
                ):
                    await self._task_dispatcher.dispatch_next_task()
