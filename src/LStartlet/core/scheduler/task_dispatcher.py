...
# 使用项目自定义日志管理器
from LStartlet.core.logger import info
...
class TaskDispatcher:
    ...
    def __init__(self, strategy: str = "round_robin", max_concurrent_tasks: int = 10):
        ...
        info(
            f"任务分发器初始化完成 (策略: {strategy}, 最大并发任务数: {max_concurrent_tasks})"
        )
    ...
    def is_busy(self) -> bool:
        """检查分发器是否繁忙"""
        return (
            self.get_active_task_count() >= self.max_concurrent_tasks
            or self.get_queue_size() > 0
        )
    ...
    async def wait_for_completion(self) -> None:
        ...
        while self.is_busy():
            ...
            while (
                ...
                and self.get_active_task_count() < self.max_concurrent_tasks
            ):
