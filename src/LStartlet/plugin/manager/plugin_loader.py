...
from LStartlet.plugin.exceptions.plugin_exceptions import PluginLoadError
from LStartlet.plugin.metadata import PluginMetadata
...
class PluginLoader:
    ...
    def load_plugin_from_wheel(
        self, wheel_path: str
    ) -> Optional[Tuple[PluginMetadata, Type[PluginBase]]]:
        """
        从 wheel 文件加载插件
        ...
        try:
            if not os.path.exists(wheel_path):
                raise PluginLoadError(
                    "unknown", f"Plugin wheel file does not exist: {wheel_path}"
                )

            if not wheel_path.endswith(".whl"):
                raise PluginLoadError(
                    "unknown", f"Plugin file must be a .whl file: {wheel_path}"
                )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not plugin_json_path:
                    raise PluginLoadError(
                        "unknown", f"plugin.json not found in wheel file: {wheel_path}"
                    )
                ...
                if not isinstance(plugin_class, type) or not issubclass(
                    ...
                    raise PluginLoadError(
                        metadata.namespace,
                        f"{class_name} is not a valid PluginBase subclass",
                    )
            ...
            raise PluginLoadError(
                "unknown", f"Failed to load plugin wheel: {wheel_path}, Error: {e}", e
            )

    def load_plugin_from_directory(
        self, plugin_dir: str
    ) -> Dict[str, Tuple[PluginMetadata, Type[PluginBase]]]:
        """
    ...
    def _load_plugin_from_source_dir(
        self, plugin_dir: str, metadata: PluginMetadata
        ...
        try:
            ...
            try:
                ...
                raise PluginLoadError(
                    metadata.namespace, f"Failed to import module {module_name}: {e}"
                )
            ...
            raise PluginLoadError(
                metadata.namespace,
                f"Failed to load plugin from source directory: {plugin_dir}, Error: {e}",
                e,
            )
    ...
    def _get_plugin_class_from_module(self, module: Any) -> Optional[Type[PluginBase]]:
        ...
        for attr_name in dir(module):
            ...
            if (
                ...
                if hasattr(attr, "__module__") and attr.__module__ == module.__name__:
                    print(
                        f"DEBUG: Found plugin class: {attr_name}, module: {attr.__module__}"
                    )
        ...
        if len(plugin_classes) == 0:
            ...
            print(
                f"DEBUG: Multiple plugin classes found, selecting first: {plugin_classes[0].__name__}"
            )
