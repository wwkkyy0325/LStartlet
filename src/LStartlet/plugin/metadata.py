    ...
    def _validate(self):
        ...
        if not self.compatibility or "min_version" not in self.compatibility:
            raise PluginLoadError(
                "metadata", "Plugin compatibility.min_version is required"
            )

        if (
            not self.entry_point
            or "module" not in self.entry_point
            or "class" not in self.entry_point
        ):
            raise PluginLoadError(
                "metadata",
                "Plugin entry_point.module and entry_point.class are required",
            )
    ...
    def is_compatible(self, metadata: PluginMetadata) -> bool:
        ...
        try:
            ...
            if not self._check_version_compatibility(
                self.current_app_version, f">={min_version}"
            ):
                return False

            # 检查最大版本限制（如果有）
            if max_version and not self._check_version_compatibility(
                self.current_app_version, f"<={max_version}"
            ):
                return False
    ...
    def check_availability(self, metadata: PluginMetadata) -> PluginAvailabilityResult:
        ...
        if metadata.dependencies:
            ...
            for dep_name, version_spec in metadata.dependencies.items():
                if not self.dependency_manager.check_dependency_availability(
