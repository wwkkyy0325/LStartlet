...
class PluginDependencyManager:
    ...
    def resolve_dependencies(self, plugin_id: str, dependencies: Dict[str, str]) -> bool:
        """
        解析并安装插件依赖
        ...
        for package_name, version_spec in dependencies.items():
            try:
                ...
                if self._install_dependency(plugin_id, package_name, version_spec):
                    ...
                    warning(f"无法安装依赖 {package_name}{version_spec}，尝试使用自包含包")
                    # 这里可以添加自包含包的处理逻辑
                    success = False
    ...
    def _install_dependency(self, plugin_id: str, package_name: str, version_spec: str) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "--target", str(plugin_dep_dir),
                "--no-user", "--no-deps",  # 不安装依赖和用户目录
                package_spec
            ]
    ...
    def cleanup_plugin_dependencies(self, plugin_id: str):
        ...
        # 清理记录
        keys_to_remove = [k for k in self.installed_deps.keys() if k.startswith(f"{plugin_id}:")]
