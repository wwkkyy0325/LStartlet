...
from plugin.metadata import PluginMetadata, PluginCompatibilityChecker, PluginAvailabilityChecker
...
class PluginManager(IPluginManager):
    ...
    def __init__(self, container: ServiceContainer, event_bus: EventBus, app_version: str = "1.0.0"):
        """
        初始化插件管理器
    ...
    def _topological_sort_plugins(self, plugins: Dict[str, PluginMetadata]) -> List[str]:
        """
        ...
        for namespace, metadata in plugins.items():
            ...
            for dep in metadata.plugin_dependencies:
                if dep in all_plugins:
                    ...
                    warning(f"Plugin {namespace} depends on {dep}, but {dep} is not available")
        ...
        if len(result) != len(all_plugins):
            ...
            raise PluginLoadError("dependency", f"Circular dependency detected among plugins: {', '.join(cycle_nodes)}")
        
        return result
    
    def _register_plugins_with_dependencies(self, plugins: Dict[str, Tuple[PluginMetadata, Type[PluginBase]]]) -> None:
        """注册插件并处理依赖关系"""
        # 先收集所有元数据用于依赖解析
        ...
        for namespace, (metadata, plugin_class) in plugins.items():
            ...
            if not self._compatibility_checker.is_compatible(metadata):
                warning(f"Plugin {namespace} is not compatible with current app version {self._app_version}")
                continue
            
            # 检查插件可用性（依赖完整性）
            availability = self._availability_checker.check_availability(metadata)
            if not availability['available']:
                warning(f"Plugin {namespace} is not available: {', '.join(availability['reasons'])}")
                continue
        ...
        for namespace in sorted_namespaces:
            ...
            info(f"Successfully loaded plugin: {namespace} ({metadata.name} v{metadata.version})")
    ...
    def _register_plugin(self, metadata: PluginMetadata, plugin_class: Type[PluginBase]) -> None:
        """注册插件到管理器"""
