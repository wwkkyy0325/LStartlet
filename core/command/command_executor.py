...
from core.error import log_error, get_error_info
...
class CommandExecutor:
        ...
        try:
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    loop.run_in_executor(self._executor, self._execute_command_sync, command, kwargs),
                ...
                timeout_msg = f"Command {command.name} timed out after {timeout_value} seconds"
                warning(timeout_msg)
    ...
    def _execute_command_sync(self, command: BaseCommand, kwargs: Dict[str, Any]) -> CommandResult:
        """
        同步执行命令（在线程池中调用）
