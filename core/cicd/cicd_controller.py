...
class CICDController:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        register_config("cicd.test.report_dir", "./test_reports", str, "测试报告输出目录")
    ...
    def run_pipeline(self, 
                     ...
                     deploy_target: Optional[str] = None) -> bool:
        """
        运行完整的CI/CD流水线
        ...
        try:
            # 在部署前进行系统检测（如果启用）
            if (get_config("cicd.system.pre_deploy_detection", True) and 
                deploy_target and self.system_config_manager):
                info("执行部署前系统检测")
                if not self.system_config_manager.setup_optimal_config():
                    warning("系统配置设置存在问题，但仍继续部署")
            
            # 检查并安装缺失的依赖（如果启用）
            if (get_config("cicd.dependencies.install_missing", True) and 
            ...
            if deploy_target:
                ...
                if not deploy_success:
                    ...
                    # 部署后系统验证（如果启用）
                    if (get_config("cicd.system.post_deploy_validation", True) and 
                        self.system_config_manager):
                        info("执行部署后系统验证")
                        # 这里可以添加健康检查、功能验证等
    ...
    def _send_notification(self, message: str, status: str) -> None:
        ...
        """
        notify_enabled = get_config("cicd.notify.on_failure", True) if status == "failure" else get_config("cicd.notify.on_success", False)
    ...
    def generate_pipeline_report(self, pipeline: Pipeline, results: Dict[str, Any]) -> str:
        """
        生成流水线执行报告
        ...
        if self.system_config_manager:
            try:
                system_info = self.system_config_manager.system_detector.detect_system_info()
