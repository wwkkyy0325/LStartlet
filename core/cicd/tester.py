...
class Tester:
    ...
    def _run_test_directory(self, test_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 保存报告
            self._save_test_report(report_data, f"test_report_{test_dir}_{start_time.strftime('%Y%m%d_%H%M%S')}.json")
            
            # 生成JUnit格式报告
            self._generate_junit_report(result, test_dir, start_time)
            
            info(f"测试完成: {report_data['total_tests']} 个测试, "
    ...
    def _generate_junit_report(self, test_result: unittest.TestResult, test_suite: str, start_time: datetime) -> str:
        """生成JUnit格式的测试报告"""
        try:
            # 创建根元素
            ...
            # 保存JUnit报告
            junit_filename = f"junit_report_{test_suite}_{start_time.strftime('%Y%m%d_%H%M%S')}.xml"
    ...
    def run_performance_tests(self, test_targets: Optional[List[str]] = None) -> Dict[str, Any]:
        """运行性能测试"""
        info("开始运行性能测试")
        ...
        info(f"性能测试完成: {results['summary']['passed']} 通过, {results['summary']['failed']} 失败")
        return results
    ...
    def run_security_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 运行安全扫描
            result = subprocess.run(
                ["bandit", "-r", self.project_root, "-f", "json", "-o", "/tmp/bandit_results.json"],
                cwd=self.project_root,
                capture_output=True,
                text=True
            )
            ...
            security_report = {
                "ran_at": datetime.now().isoformat(),
                "issues_found": scan_results.get("metrics", {}).get("_totals", {}).get("SEVERITY.HIGH", 0),
                "details": scan_results
            }
            
            # 保存安全扫描报告
            report_path = os.path.join(self.report_dir, f"security_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
            with open(report_path, 'w', encoding='utf-8') as f:
