...
class Deployer:
    ...
    def rollback(self, target_env: str, version: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # 确定要回滚到的版本
            if version:
                rollback_deployment = next((d for d in history if d['version'] == version), None)
            else:
                # 回滚到上一个版本
                history_sorted = sorted(history, key=lambda x: x['deployed_at'], reverse=True)
                if len(history_sorted) < 2:
                    error(f"没有足够的部署历史来进行回滚")
                    return False
            ...
            if success:
                # 记录回滚操作
                self._record_rollback(target_env, rollback_deployment['version'], rollback_artifact)
    ...
    def _get_deployment_config(self, target_env: str) -> Optional[Dict[str, Any]]:
        """获取部署配置"""
        config_file = os.path.join(self.project_root, "deployment", f"{target_env}_config.yaml")
    ...
    def _execute_deployment(self, target_env: str, artifact_path: str, config: Dict[str, Any], include_models: Optional[bool] = None) -> bool:
        ...
        try:
            ...
            if config.get("backup_before_deploy", False):
                backup_path = f"{target_path}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            if config.get("notify_on_deploy", False):
                self._send_deploy_notification(target_env, os.path.basename(artifact_path))
    ...
    def _run_health_check(self, target_env: str, deploy_dir: str) -> bool:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("health_check_app", main_app_path)
    ...
    def _send_deploy_notification(self, target_env: str, artifact_name: str) -> None:
        ...
        # 为了简化，这里只是记录日志
        notification_msg = f"部署成功: {artifact_name} -> {target_env} at {datetime.now()}"
    ...
    def _record_rollback(self, target_env: str, version: str, artifact_path: str) -> None:
        """记录回滚操作"""
        ...
        # 保存到专门的回滚历史文件
        rollback_history_file = os.path.join(self.project_root, "deployment", "rollback_history.yaml")
