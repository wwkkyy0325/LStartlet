...
class DependencyResolver:
                    ...
                    try:
                        ...
                        for dep in file_deps:
                            ...
                            if main_package in self.stdlib_modules:
                                ...
                                init_path = os.path.join(project_module_path, '__init__.py')
                                
                                if os.path.exists(project_module_path) or os.path.exists(init_path):
    ...
    def _extract_imports(self, file_path: str) -> Set[str]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.module:  # 检查是否有模块名（例如，from . import xxx 没有模块名）
        ...
        try:
            ...
            for pkg in all_packages:
                ...
                for req_pkg in required_packages:
                    ...
                    if pkg_name == req_normalized or pkg_name.startswith(req_normalized + '-'):
    ...
    def compare_with_requirements(self, requirements_file: str = "requirements.txt") -> Dict[str, Set[str]]:
        """
        ...
        return {
            'in_project_not_in_req': current_deps - required_deps,  # 在项目中但不在req文件中的
            'in_req_not_in_project': required_deps - current_deps,  # 在req文件中但不在项目中的
