...
from core.error.exceptions import ConfigError
...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self._system_config_path = os.path.join(self._project_root, 'system_config.yaml')
    ...
    def _initialize_config(self):
        ...
        if os.path.exists(self._system_config_path):
            ...
            if system_config:
                self._flatten_and_merge_config(system_config, source='system', protected=True)
    ...
    def _flatten_and_merge_config(self, new_config: Dict[str, Any], source: str, protected: bool = False):
        """扁平化并合并配置字典"""
        flat_config = self._flatten_dict(new_config)
    ...
    def reset_config(self, key: str) -> bool:
        ...
        if key in system_defaults:
            old_value = self._config.get(key)
            self._config[key] = system_defaults[key]
            self._config_sources[key] = 'system'
            self._notify_listeners(key, old_value, system_defaults[key])
            return True
    ...
    def register_config(self, key: str, default_value: Any, value_type: Type, 
                       description: str = "", plugin_name: Optional[str] = None) -> bool:
        """注册新的配置项"""
        ...
        if not isinstance(default_value, value_type):
            error(f"配置项 {key} 的默认值类型不匹配: 期望 {value_type}, 实际 {type(default_value)}")
            return False
    ...
    def register_config_with_source(self, key: str, default_value: Any, value_type: Type,
                                  description: str = "", source: str = "external", 
    ...
    def load_from_file(self, filename: str) -> bool:
        ...
        try:
            ...
            if config_data:
                self._flatten_and_merge_config(config_data, source='user', protected=False)
                return True
            return False
        except Exception as e:
    ...
    def load_plugin_config(self, namespace: str, config_path: str):
        """加载插件配置文件"""
        try:
            ...
            if config_data:
                ...
                self._flatten_and_merge_config(namespaced_config, source=f'plugin:{namespace}', 
                                             protected=False)
                ...
                # 如果没有配置文件，创建空配置字典
                empty_config = {}
                self._merge_config(empty_config, source=f'plugin:{namespace}', 
                                 protected=False, namespace=namespace)
                self._plugin_namespaces.add(namespace)
        except Exception as e:
    ...
    def _merge_config(self, new_config: Dict[str, Any], source: str, protected: bool = False, 
        ...
        if namespace:
            ...
            for key, value in new_config.items():
                namespaced_key = f"{namespace}.{key}"
                namespaced_config[namespaced_key] = value
            self._flatten_and_merge_config(namespaced_config, source=source, protected=protected)
        else:
            self._flatten_and_merge_config(new_config, source=source, protected=protected)