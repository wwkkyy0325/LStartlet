...
def with_logging(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            # 获取日志函数
            log_func = getattr(
                __import__("core.logger"), level, __import__("core.logger").info
            )
...
def cached_async(maxsize: int = 128, ttl: Optional[float] = None, typed: bool = False):
    """异步缓存装饰器"""
...
def require_permission(
    required_level: PermissionLevel, error_message: str = "Insufficient permissions"
):
    """权限检查装饰器"""
...
def monitor_metrics(metric_name: str, include_labels: bool = True):
    """监控指标装饰器"""
...
def monitor_metrics_async(metric_name: str, include_labels: bool = True):
    """异步监控指标装饰器"""
...
def validate_config(
    config_key: str, validator_func: Optional[Callable[[Any], bool]] = None
):
    """
    配置验证装饰器
    在函数执行前验证配置项
